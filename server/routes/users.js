...
function publicUser(u) {
  return {
    ...
    access: Array.isArray(u.access) ? u.access : (u.role === 'Super Admin' ? ['*'] : [])
  };
}
...
router.post('/', requireRole('Super Admin', 'Admin'), (req, res) => {
  ...
  logAction(store, req.user.email, 'user.create', email);
  db.saveStore();
...
router.put('/:id', requireRole('Super Admin', 'Admin'), (req, res) => {
  ...
  // Cannot suspend or deactivate yourself
  if (String(req.user.id) === String(id) && b.status && b.status !== 'Active') {
  ...
  if (Array.isArray(b.access)) u.access = b.access;
  if (b.password && String(b.password).length >= 6) {
    u.password = bcrypt.hashSync(b.password, 10);
  }

  logAction(store, req.user.email, 'user.update', u.email);
  db.saveStore();
...
router.delete('/:id', requireRole('Super Admin'), (req, res) => {
  ...
  logAction(store, req.user.email, 'user.delete', user.email);
  db.saveStore();
  res.json({ ok: true });
});
